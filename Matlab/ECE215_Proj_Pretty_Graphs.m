...
plot(k,B(:,3));
plot(k,D(:,3));
legend('Actual_X','Desired_X','Actual_Y','Desired_Y','Actual_Z','Desired_Z',Location='bestoutside');
xlabel('Time [s]');
ylabel('Position [M]');
title('X,Y,Z Desired vs Actual Positions')
hold off
pause(1);
figure(2);
...
% 
for y = 2:length(B)/10
    plot3(B((y-1)*10:y*10,1),B((y-1)*10:y*10,2),B((y-1)*10:y*10,3), 'r');%,LineStyle='--');
    hold on
    plot3(D((y-1)*10:y*10,1),D((y-1)*10:y*10,2),D((y-1)*10:y*10,3),'b',LineStyle='--');
    pause(0.03);
